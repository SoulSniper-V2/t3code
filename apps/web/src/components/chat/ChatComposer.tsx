...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const mentionableThreadItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    const shellById = new Map(
      [
        ...environmentThreadShells.filter(
    ...
    return candidates
      ...
      .map((thread) => {
        ...
        return {
          ...
          type: "thread",
          thread: { environmentId, threadId: thread.id },
          label: thread.title,
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "thread") {
        ...
        const record = threadContextRecord(item.thread, item.label);
      ...
      {composerControlsCompact ? (
        ...
        <>
          {restingBlockDefs.map((def, index) => {
            ...
            const iconOnly = index >= restingBlockDefs.length - iconOnlyBlockCount;
            return (
              <div
                key={def.id}
