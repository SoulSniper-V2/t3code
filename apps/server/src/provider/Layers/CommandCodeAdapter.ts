  ...
  return Effect.gen(function* () {
    ...
    const runTurnRaw = (input: {
      ...
      Effect.gen(function* () {
        ...
        // exit as a PlatformError rather than a code, and every interrupt
        // kills the child — so the exit wait must never fail the turn. The
        // interruption check and result framing below own that decision.
        ...
        // Guarded: a replacement session must not inherit a stale run's id.
        ...
        // map to the canonical error classes. -1 is the sentinel for a
        // signal-killed child that something other than our interrupt ended.
        const stderrMessage = stderrTail.trim().split(/\r?\n/).slice(-3).join("\n").trim();
        const detail =
          resultSubtype === "error"
            ? `Command Code turn failed${exitCode !== 0 ? ` (exit ${exitCode})` : ""}`
            : exitCode === -1
              ? "Command Code was terminated by signal"
