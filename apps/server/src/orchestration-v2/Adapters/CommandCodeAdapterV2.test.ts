...
function readInvocations(path: string): Array<{ argv: string[]; prompt: string }> {
...
function makeTurnInput(input: {
  ...
  return {
    ...
    modelSelection: { instanceId: input.instanceId, model: "deepseek/deepseek-v4-flash" },
    runtimePolicy: {
...
it.layer(CommandCodeAdapterV2TestLayer)("CommandCodeAdapterV2 (mock CLI)", (it) => {
  it.effect("streams V2 artifacts and resumes the native Command Code session", () =>
    Effect.gen(function* () {
      ...
      const storedThread = firstEvents
        .filter((event) => event.type === "provider_thread.updated")
        .at(-1);
  ...
  it.effect("interrupts an active subprocess and does not advertise unsupported operations", () =>
    Effect.gen(function* () {
      ...
      expect(CommandCodeProviderCapabilitiesV2.tools.supportsMcpTools).toBe(false);
    }).pipe(Effect.scoped),
  );
});
