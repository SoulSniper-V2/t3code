...
export class DesktopWindow extends Context.Service<
  DesktopWindow,
  {
    ...
    readonly showConnectingSplash: (target?: "desktop" | "wsl") => Effect.Effect<void>;
...
// A self-contained startup splash, shown before the local backend is ready.
// Inlined as a data URL so it needs no bundled asset or backend — pure CSS, no JS.
function buildConnectingSplashDataUrl(
  ...
  const message = target === "wsl" ? "Connecting to WSL…" : "Starting T3 Code…";
...
export const make = Effect.gen(function* () {
  ...
  // The transient startup splash, tracked separately so it is never mistaken
  // for the real main window.
  ...
  const showConnectingSplash = Effect.fn("desktop.window.showConnectingSplash")(
    (target: "desktop" | "wsl" = "wsl") =>
      Effect.gen(function* () {
        ...
        void splash.loadURL(buildConnectingSplashDataUrl(shouldUseDarkColors, target));
        yield* logWindowInfo("startup splash shown", { target });
      }).pipe(
