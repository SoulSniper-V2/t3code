...
- `.github/workflows/desktop-macos-preview-publish.yml` runs on `workflow_run` from `main`. It
  ...
  PR commit, as data, so the preview reflects the PR's app version and cloud configuration. A PR
  that changes packaging must use the `channel=preview` release train above instead.
...
1. The Apple Developer account has rights to create Developer ID certificates.
2. Create a `Developer ID Application` certificate.
3. Export the certificate and private key as a `.p12` from Keychain.
4. Base64-encode the `.p12` and store it as `CSC_LINK`.
5. Store the `.p12` export password as `CSC_KEY_PASSWORD`.
6. In App Store Connect, create a team API key and store its contents, Key ID, and Issuer ID as the
   three `APPLE_API_*` secrets.
7. Re-run a release and confirm the macOS artifacts are signed and notarized.

The workflow writes `APPLE_API_KEY` to a temporary `AuthKey_<id>.p8` file while packaging.
Developer ID signing and notarization do not require an Associated Domains provisioning profile.
...
- macOS build unsigned when expected signed:
  - Check the five Apple signing and notarization secrets are populated and non-empty.
