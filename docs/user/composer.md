...
Type `/` in an empty composer to open the command menu. After other prompt text,
`/` opens a skill-only menu. Type `$` to find and add a skill anywhere. On mobile,
both are also available before starting a thread on **New task**.

The slash menu in an empty composer also includes skills unless you turn off
**Settings → General → Show skills in slash menu**. Inline slash searches always
show skills enabled for the provider.

Commands are offered before other prompt text. Provider commands must start the
message to run. Skill mentions work anywhere in the message.
